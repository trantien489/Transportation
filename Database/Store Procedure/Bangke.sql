...
begin
	...
	DistanceDescription varchar(15), CapacityType varchar(20), Money money, Report nvarchar(200), DriverPrimary nvarchar(100), DriverSecondary nvarchar(100) );

	declare pointer cursor for			
		select T.TransportDate, C.CarNumber, T.CompanyIds, CP.Type, T.Money, T.Report, D1.Name, D2.Name from Transportation T 
	...
	open pointer
	declare 
		@TransportDate date, @CarNumber varchar(15),@CompanyIds varchar(max), @CapacityType varchar(20),  @Money money, @Report nvarchar(200), @DriverPrimary nvarchar(100), @DriverSecondary nvarchar(100);
		fetch next from pointer into @TransportDate, @CarNumber, @CompanyIds, @CapacityType, @Money, @Report, @DriverPrimary, @DriverSecondary;
	...
	begin
		...
		insert into @BangkeTemp values(CONVERT(VARCHAR(10), @TransportDate, 103), @CarNumber, @CompanyName, @CompanyDistance, @DistanceDescription, @CapacityType, @Money, @Report, @DriverPrimary, @DriverSecondary);
