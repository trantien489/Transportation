...
import { getAllPermissionAction, changeStatusPermissionAction, deletePermissionAction } from '../../../actions/permission';
...
class Permission extends Component {
    constructor(props) {
        super(props);
        this.state = {
            isEnableFilter: getSessionStorage(commonConstant.FILTER_PERMISSION) === 'true' ? true : false,
        };
        applyCheckAuthorized();
    }
    componentDidMount() {
        this.props.getAllPermissionAction();
    }
    ...
    changeStatusAction = (item) => {
        _handleItem = item;
        this.props.changeStatusPermissionAction(item.Id);
    }
    deleteAction = (item) => {
        _handleItem = item;
        this.props.deletePermissionAction(item.Id);
    }
    editAction = (item) => {
        const { history } = this.props;
        if (history) {
            history.push('permission/' + item.Id);
        }
    }
    render() {
        ...
        if (getAllModel.isLoading) {
            ...
            if (hasRecordsActionReducer(getAllModel)) {
                return <GridView records={getAllModel.responseData.Data.Records} t={t} isLoadingOver={isLoadingOver}
                    keyFields={key.permission} tableName="PERMISSION"
...
const mapStateToProps = state => ({
    getAllModel: state.getAllPermissionReducer,
    changeStatusModel: state.changeStatusPermissionReducer,
    deleteModel: state.deletePermissionReducer,
});
//Phát đi tính hiệu thông qua action (để lấy data từ api)
const mapDispatchToProps = {
    getAllPermissionAction,
    changeStatusPermissionAction,
    deletePermissionAction,
};
export default translate()(connect(mapStateToProps, mapDispatchToProps)(Permission));